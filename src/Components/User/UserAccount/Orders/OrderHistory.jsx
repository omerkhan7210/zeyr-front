...
import { OrderRowsMyAccount } from './OrderRowsMyAccount';
...
export const OrderHistory = ({userid}) => {
 ...
 // State to store the most recent order
 const [mostRecentOrder, setMostRecentOrder] = useState(null);

  // Effect to find and set the most recent order when the component mounts or when orders change
  useEffect(() => {
    if (orders.length > 0) {
      // Find the most recent order based on order_date
      const recentOrder = orders.reduce((mostRecent, order) => {
        return order.order_date > mostRecent.order_date ? order : mostRecent;
      }, orders[0]);

      // Set the most recent order in the state
      setMostRecentOrder(recentOrder);
    }
  }, [orders]); // Trigger the effect whenever orders change


  return (
    loading ? <p className='cart-sidebar-loading'> <img src='/images/loadingGIF.gif'  width={"30px"}/></p> : 
       
<div className="order-history-my-account light flex-c ">
  ...
  { orders  && orders.length > 0 ? (
    <>
    {mostRecentOrder &&
    <MostRecentOrder mostRecentOrder={mostRecentOrder}/>
    }

      <div className="tableView products-area-wrapper ">
        <h6 className='variant-heading' style={{marginLeft:0,marginBottom:'1rem'}}>Previous Orders</h6>
      <div className="products-header">
        ...
        <div className="product-cell price">Order Amount<button className="sort-button">
          ...
          {mostRecentOrder && 
          orders 
          .filter((order) => (order.order_id !== mostRecentOrder.order_id))
          .map((order) => (
            <>

            {
            order.addresses[0] &&
            order.addresses[0].user_id == userid ?
           <OrderRowsMyAccount order={order} key={order.id}/>
          : null}
          </>
          ))
